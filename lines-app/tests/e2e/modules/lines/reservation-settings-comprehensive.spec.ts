  ...
  test.describe("Error Scenarios", () => {
    test("should show error for excluded line", async ({ page, context }) => {
      ...
      if (await settingsSection.isVisible()) {
        ...
        await expect(page.getByText(/ליין.*לא מורשה|line.*excluded|לא ניתן/i)).toBeVisible({
          timeout: 5000
        });
      }
    });
    ...
    test("should show error when reservations disabled for venue", async ({ page, context }) => {
      ...
      if (await settingsSection.isVisible()) {
        ...
        await expect(page.getByText(/הזמנות.*מושבתות|reservations.*disabled/i)).toBeVisible({
          timeout: 5000
        });
      }
    });
