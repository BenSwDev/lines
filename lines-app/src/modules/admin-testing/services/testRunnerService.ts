...
export async function runVitestTests(testType: "unit" | "integration"): Promise<{
  total: number;
  passed: number;
  failed: number;
  skipped: number;
  ...
  const configFile = testType === "unit" ? "vitest.config.unit.ts" : "vitest.config.integration.ts";
  ...
  try {
    // In Vercel, use npx or direct node_modules path
    // Check if we're in Vercel (has VERCEL env var) or production
    const isVercel = !!process.env.VERCEL;
    const vitestCommand = isVercel
      ? "npx --yes vitest" // Use npx with --yes to auto-install if needed
    ...
    const { stdout } = await execAsync(
      `${vitestCommand} run --config ${configFile} --reporter=json --reporter=verbose`,
      {
        cwd: process.cwd(),
        env,
